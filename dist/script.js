...
async function deleteFileOrDir(filePath) {
  try {
    ...
    if (stat.isDirectory()) {
      const files = await import_promises.default.readdir(filePath);
      for (const file of files) {
        const fullPath = import_node_path.default.join(filePath, file);
        await deleteFileOrDir(fullPath);
      }
      await import_promises.default.rmdir(filePath);
...
async function start() {
  try {
    ...
    const { title } = await (0, import_prompts.default)({
      ...
      type: "text",
      message: "\u8BF7\u8F93\u5165\u9879\u76EE\u6807\u9898:",
      initial: result.templateType.type === "template-vanilla" ? "My-App" : `My-${capitalizeFirstLetter(result.templateType.type)}-App`
    });
