...
var require_Int64 = __commonJS({
  "node_modules/.pnpm/node-int64@0.4.0/node_modules/node-int64/Int64.js"(exports2, module2) {
    ...
    Int64.prototype = {
       ...
       */
      _2scomp: function() {
        var b = this.buffer, o2 = this.offset, carry = 1;
        for (var i2 = o2 + 7; i2 >= o2; i2--) {
      ...
      setValue: function(hi, lo) {
        ...
        var b = this.buffer, o2 = this.offset;
      ...
      toNumber: function(allowImprecise) {
        var b = this.buffer, o2 = this.offset;
        var negate = b[o2] & 128, x = 0, carry = 1;
...
var require_es5 = __commonJS({
  "node_modules/.pnpm/bluebird@3.7.2/node_modules/bluebird/js/release/es5.js"(exports2, module2) {
    ...
    if (isES5) {
      ...
      ObjectGetDescriptor = function(o2, key) {
        return { value: o2[key] };
      };
      ObjectDefineProperty = function(o2, key, desc) {
        o2[key] = desc.value;
        return o2;
      };
...
function get(templateName, targetName) {
  if (import_node_fs.default.existsSync(targetName)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    import_node_https.default.get(url, (res) => {
      ...
      res.pipe(import_unzipper.default.Extract({ path: `./${templateName}` })).on("close", async () => {
        try {
...
async function moveFiles(name) {
  const nestedDir = import_node_path.default.join(`./${name}`, `${name}-master`);
  const files = await readdir(nestedDir);
  for (const file of files) {
    const oldPath = import_node_path.default.join(nestedDir, file);
    const newPath = import_node_path.default.join(`./${name}`, file);
  ...
  import_node_fs.default.rmdirSync(nestedDir, { recursive: true });
}
async function moveDir(src, dest) {
  const files = await readdir(src);
  await import_node_fs.default.promises.mkdir(dest, { recursive: true });
  for (const file of files) {
  ...
  import_node_fs.default.rmdirSync(src, { recursive: true });
}
var o = `output-1722702872026`;
console.log(123456);
get("template-vanilla", o).then(() => {
  changePackageName(o);
}).catch((err) => {
  console.error("Error:", err);
});
function changePackageName(target) {
  const cwd = import_node_process.default.cwd();
  const packageFilePath = `${cwd}\\${target}\\package.json`;
  const s = import_node_fs.default.readFileSync(packageFilePath, "utf-8");
  console.log(s);
  const packageName = JSON.parse(s);
  packageName.name = target;
  import_node_fs.default.writeFileSync(packageFilePath, JSON.stringify(packageName, null, 2));
  console.log("Download, extraction, and reorganization complete");
}
async function deleteFile(filePath) {
  try {
    await import_node_fs.default.unlinkSync(filePath);
